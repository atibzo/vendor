// src/pages/TripVendors.tsx
import { useMemo, useState } from "react";
import { useLocation, useParams } from "react-router-dom";
import { DEMO_BATCHES, ServiceType } from "../utils/data";
...
export default function TripVendors() {
  ...
  const preselectVendor = params.get("selectVendor");
  ...
  const [note, setNote] = useState(
    "Check-in window 12:00–14:00. Need rooming list D-1."
  );
  ...
  const vendors = useMemo(
    () =>
      getVendors().filter(
        (v) =>
          (service ? v.type === service : true) &&
          (v.name.toLowerCase().includes(search.toLowerCase()) ||
            v.areas.join(" ").toLowerCase().includes(search.toLowerCase()))
      ),
    [service, search]
  );

  const batch = batches.find((b) => b.id === selected);
  const preselectedVendorObj = vendors.find((v) => v.id === preselectVendor);

  function assign(vendorId: string) {
    if (!batch) return;
    const existing = batch.vendorAssignments.find((a) => a.type === service);
    if (existing) existing.vendorId = vendorId;
    else batch.vendorAssignments.push({ type: service, vendorId, notes: note });
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 divide-y divide-zinc-800">
          {batches.map((b) => (
            <button
              ...
              className={`w-full text-left px-4 py-3 hover:bg-zinc-900/40 ${
                b.id === selected ? "bg-zinc-900/50" : ""
              }`}
            >
              ...
              <div className="text-xs text-zinc-400">
                Vendor:{" "}
                {b.vendorAssignments[0]?.vendorId
                  ? vendors.find((v) => v.id === b.vendorAssignments[0].vendorId)
                      ?.name
                  : "— none —"}
              </div>
            </button>
      ...
      <div className="lg:col-span-2">
        <div className="flex items-center justify-between mb-2">
          <div className="text-sm text-zinc-400">
            Assign Vendor to <span className="text-zinc-200">{batch?.name}</span>
          </div>
          <div className="flex gap-2">
            <button
              className="rounded-xl border border-zinc-700 px-3 py-2"
              onClick={saveCopyWhatsApp}
            >
              Save & Copy WhatsApp
            </button>
            <button
            ...
            <a
              className="rounded-xl border border-zinc-700 px-3 py-2"
              href={batch ? `/public/vendor/${batch.tripId}/${batch.id}` : "#"}
              target="_blank"
            >
              View Vendor View
            </a>
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <select
              className="w-full rounded-xl bg-zinc-900/70 border border-zinc-800 px-3 py-2"
              value={service}
          ...
          <div>
            <div className="text-xs text-zinc-400 mb-1">
              Notes to vendor (internal → prefill msg)
            </div>
            <input
              className="w-full rounded-xl bg-zinc-900/70 border border-zinc-800 px-3 py-2"
              value={note}
        ...
        <div className="mb-2 flex gap-2">
          <input
            className="w-80 rounded-xl bg-zinc-900/70 border border-zinc-800 px-3 py-2"
            placeholder="Search vendor name / area"
            value={search}
          ...
          {vendors.map((v) => (
            <div
              ...
              className={`rounded-xl border ${
                preselectedVendorObj?.id === v.id
                  ? "border-lime-400"
                  : "border-zinc-800"
              } p-3 hover:bg-zinc-900/50`}
            >
        ...
        <div className="mt-6 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4">
          ...
          <ul className="text-sm">
            {batch?.vendorAssignments.length ? (
              batch.vendorAssignments.map((a) => (
                <li key={a.type}>
                  {a.type} →{" "}

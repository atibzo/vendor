...
export default function BatchDetail(){
  ...
  const b = DEMO_BATCHES.find(x=>x.id===batchId);
  if(!b) return <div className='text-zinc-400'>Batch not found.</div>
  const guests = DEMO_GUESTS;
  const cap1 = DEMO_CAPTAINS.find(c=>c.id===b.captainPrimaryId);
  const cap2 = DEMO_CAPTAINS.find(c=>c.id===b.captainBackupId);
  return (
    <div>
      <h2 className='text-lg font-semibold'>{b.name}</h2>
      <div className='text-sm text-zinc-400'>{b.start} → {b.end} • Status: {b.status}</div>
      <div className='mt-4 grid md:grid-cols-2 gap-4'>
        <div className='rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4'>
          <div className='text-sm text-zinc-400'>Captains</div>
          <div>{cap1?.name} {cap1?.phone?`• ${cap1.phone}`:''}</div>
          {cap2 && <div className='text-zinc-300'>Backup: {cap2.name} {cap2.phone?`• ${cap2.phone}`:''}</div>}
        </div>
        <div className='rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4'>
          <div className='text-sm text-zinc-400'>Transfer plan</div>

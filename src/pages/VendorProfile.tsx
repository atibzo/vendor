      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-2">
          <span className="inline-flex items-center rounded-full border border-zinc-700 px-2 py-0.5 text-xs text-zinc-300">
            {v.flags?.preferred ? "Preferred" : "—"}
          </span>
          <button className="rounded-xl border border-zinc-700 px-3 py-2">
            Assign to Group
          </button>
        </div>
      </div>
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 mb-4">
            <div className="grid md:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <div>
                  Primary: {v.primary.name}{" "}
                  {v.primary.phone ? `• ${v.primary.phone}` : ""}
                </div>
                {v.backup && (
                  <div>
                    Backup: {v.backup.name}{" "}
                    {v.backup.phone ? `• ${v.backup.phone}` : ""}
                  </div>
                )}
              </div>
              <div>
        ...
        <div className="lg:col-span-1">
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 mb-4">
            ...
            {(v.docs || []).map((d, i) => (
              ...
              >
                <div>
                  {d.type} • {d.ref || ""} • {d.expiry || ""}
                </div>
                <button className="rounded-xl px-3 py-2 text-zinc-300 hover:bg-zinc-900/60">
                  Replace
                </button>
              </div>
            ))}
